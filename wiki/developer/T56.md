...
5. The package `model` contains the core domain model of the module. The subpackage `dto` holds all the Data Transfer Objects used for client and API interactions.
6. The package `service` defines utility service used by the game engine, such as `FIleOperationUtils`, which handles the logging of each compilation executed by the users,
   and `AchievementService`, which handle the logic to verify if the player, during the play, has unlocked new achievements;
